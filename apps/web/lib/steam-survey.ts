...
function parseSurvey(html: string, nameBySlug: Map<string, string>): GamersRating | null {
  ...
  while ((row = rowRe.exec(block)) !== null) {
    ...
    const name = mapped.slug ? (nameBySlug.get(mapped.slug) ?? mapped.name) : mapped.name;
...
export async function getGamersRating(): Promise<GamersRating | null> {
  try {
    const [res, distros] = await Promise.all([
      fetch(SURVEY_URL, {
    ...
    const nameBySlug = new Map(distros.map((d) => [d.slug, d.name]));
    return parseSurvey(await res.text(), nameBySlug);
  } catch {
    return null;
  }
}
